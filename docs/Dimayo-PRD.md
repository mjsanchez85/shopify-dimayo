...
Lacquer **no** es color de esquema — no tiene rol de fondo/texto propio, se aplica vía CSS custom en `assets/base.css`, únicamente en los 4 puntos de la tabla de arriba.

### 2.2 Tipografía

*Se invierte el patrón habitual (serif grande + sans chico). Acá el display sans funciona como cartel de señalética de sala, y el serif queda para el texto largo tipo cédula de museo — el objeto se anuncia con una tipografía de señalética y se explica con una de lectura.*

| Rol | Fuente | Peso / estilo | Uso |
|---|---|---|---|
| Display | **Bricolage Grotesque** | 600–700, mayúsculas o versalitas, tracking amplio (0.04–0.08em) | H1, H2, nombres de producto, nav |
| Body / texto curatorial | **Newsreader** | 400 regular, itálica para el texto "por qué lo elegimos" | Párrafos largos, descripciones, ficha curatorial |
| UI / labels / botones | **Bricolage Grotesque** | 500–600, uppercase, tracking 0.1em | Botones, labels, filtros |
...
| Body / descripción | 16–17px | 15px | Newsreader 400 |
| Texto curatorial ("por qué lo elegimos") | 16–17px, itálica | 15px, itálica | Newsreader 400 itálica |
| Label / botón / nav | 11–12px, uppercase, tracking 0.1em | igual | Bricolage Grotesque 600 |
| Caption / ficha técnica | 12–13px | igual | Newsreader 400, no itálica — la ficha técnica se lee como dato, no como confidencia |

**Nota de implementación:** ni Bricolage Grotesque ni Newsreader están, previsiblemente, en la librería de Shopify Fonts que expone el `font_picker` nativo (`type_header_font` / `type_body_font` en `config/settings_schema.json`). Confirmar antes de asumir lo contrario. Camino esperado: self-hostear ambas como `.woff2` en `assets/`, declarar `@font-face` en `assets/base.css`, y sobrescribir `--font-heading-family` / `--font-body-family` (generadas por Dawn en `snippets/theme-styles-variables.liquid`) sin editar ese snippet directamente — ver reglas en `CLAUDE.md`.
...
| Etiqueta curatorial | Pequeño texto Newsreader itálica bajo cada producto en grilla, tipo cédula de museo — 1 línea, ej. "Pieza 4 de la selección actual" o el material en una palabra. No es descripción, es contexto. |
...
| 5 | SKU | bloque `sku` | Newsreader, caption, sin protagonismo. |
...
| 11 | Por qué la elegimos | bloque de texto libre (nuevo — no existe en Opción A) | El bloque diferencial de este sistema: 2–4 líneas en Newsreader itálica explicando el criterio de selección de esta pieza puntual, no una descripción de features. Ej: "La elegimos por el mecanismo de altura, no por el diseño — es el único que no se afloja en un año de uso diario." |
| 12 | Descripción funcional | bloque `description` | Newsreader regular, no itálica — datos de uso, separados del bloque curatorial (11) que sí es itálica/voz de marca. |
| 13 | Especificaciones técnicas | bloque `collapsible_tab` × 1 por categoría | Newsreader, sin itálica — la ficha técnica se lee como dato. Accordion en mobile, igual criterio que en la Opción A. |
...
| 7 | Nota de envío general | texto libre, Newsreader itálica Ash | Sin tono de disculpa. |
...
| Legal | © año Dimayo — Preferencias de Cookies — Términos y Condiciones — Política de Privacidad, Newsreader itálica pequeña, Ash |
...
- [ ] Confirmar disponibilidad de Bricolage Grotesque / Newsreader en `font_picker` de Shopify Fonts; casi seguro requiere self-host — subir `.woff2` a `assets/`, declarar `@font-face` en `assets/base.css`, sobrescribir `--font-heading-family` / `--font-body-family`.
